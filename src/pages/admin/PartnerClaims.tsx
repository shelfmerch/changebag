...
const PartnerClaims = () => {
  ...
  useEffect(() => {
    const fetchClaims = async () => {
      ...
      try {
        setLoading(true);
        console.log(`Fetching partner claims from ${config.apiUrl}/claims/source/PARTNER_API`);
        const response = await axios.get(`${config.apiUrl}/claims/source/PARTNER_API`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        
        console.log('partner claims data:', response.data);
        ...
        console.error('Error fetching partner claims:', err);
        setError(err.response?.data?.message || 'Failed to load partner claims');
        setLoading(false);
      }
    };
